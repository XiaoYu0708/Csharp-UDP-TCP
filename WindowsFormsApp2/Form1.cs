    ...
    public partial class Form1 : Form
    {
        ...
        private string MyIP()
        {
            ...
            foreach (IPAddress it in ip)
            {
                ...
                {
                    return it.ToString(); //如果是 IPv4 回傳此 IP 字串
                }
            }
            return ""; //找不到合格 IP 回傳空字串
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Th_Svr = new Thread(ServerSub); //宣告監聽執行緒(副程式 ServerSub)
                ...
                try //用 Sck 來接收此客戶訊息，inLen 是接收訊息的 byte 數目
                {
                    ...
                    switch (Cmd) //依據命令碼執行功能
                    {
                        ...
                        case "9":
                            ...
                            listBox1.Items.Remove(Str); //自上線者名單移除 Name
        ...
        private string OnlineList()
        {
            ...
            {
                L += listBox1.Items[i]; //逐一將成員名單加入 L 字串
                                        //不是最後一個成員要加上","區隔
        ...
        {
            byte[] B = Encoding.Default.GetBytes(Str); //訊息轉譯為 byte 陣列
            Socket Sck = (Socket)HT[User]; //取出發送對象 User 的通訊物件
        ...
        {
            byte[] B = Encoding.Default.GetBytes(Str); //訊息轉譯為 Byte 陣列
            foreach (Socket s in HT.Values) //HT 雜湊表內所有的 Socket
